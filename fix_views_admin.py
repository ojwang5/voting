# Fix views_admin.py by rewriting it from scratch
import textwrap
...
def create_election(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.error(request, f"Invalid date/time format. Error: {str(e)}")
            return render(request, "polls/create_election.html", {"positions": positions, "selected_position_ids": selected_positions})
        if start_dt >= end_dt:
        ...
        try:
            election = Election.objects.create(
                ...
                eligible_ranks=eligible_ranks, eligible_stations=eligible_stations, created_by=request.user
            )
            positions_added = 0
